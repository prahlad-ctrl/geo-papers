...
def random_walk(G: nx.Graph, start_node: int, walk_length: int)->list[str]:
    walk = [start_node]
    for _ in range(walk_length-1):
        current = walk[-1] # to get last element (where it ended up)
...
def generate_walks(G: nx.Graph, num_walks: int, walk_length: int)->list[str]:
    nodes = list(G.nodes()) 
    ...
    for walk_idx in range(num_walks):
        ...
        for node in nodes:
            w = random_walk(G, node, walk_length)
            walks.append(w)
...
def train_deepwalk(walks: list[list[str]], embed_dim = 64, window = 5, workers = 4, epochs = 5)->Word2Vec:
...
def main():   
    ...
    model = train_deepwalk(walks, embed_dim=64, window=5, workers=5, epochs=5)
